 ...
 * =======================使用方式=====================
	1、初始化；
	var customTable = new queryTablebysql();
...
//	this.tableData = null;
	...
	this.ajaxRequest = function(ops) {
		...
		if(this.tableData != null){
			this.ajaxRequestData(this.tableData);
		}else{
			$.ajax({
				url : ops["url"],
				type : 'POST',
...
//			return false;
	...
	this.init = function(tData) {
		...
		//传入的表头数据
		if(tData['headData']){
			this.headData = tData['headData'];
		}
		
		//传入的表数据
		if(tData['tableData']){
			this.tableData = tData['tableData'];
		}
			...
			if(info["renderer"]=="onActionRenderer"){
				...
				if(reportFlag && i == 1){
					...
					rowhtml+='<button style="color:blue;" onclick="jumpDocumentDetails('+OTHERSYSTEMMAINID
						+','+COMMISSITION_TYPE+','+STATUS+')">'+cellValue+'</button>';
...
//				rowhtml+='<div class="hide-text"><i class="jt-up"></i>'+cellValue+'</div>'+'</div>';
	...
	this.renderUrl=function(tempBean,colIdx,cellValue) {
		...
		if(info.name){
			...
			if(href_type=='1'){
				rst = '<a class="LinkWord" href="javascript:queryTablebysql.openWin(\'' + url + '\')">'+value+'</a>';
			}else if(href_type=='2'){
				rst = '<a class="LinkWord" href="javascript:queryTablebysql.jumpPage(\'' + url + '\')">'+value+'</a>';
			}else if(href_type=='3'){
				rst = '<a class="LinkWord" href="javascript:queryTablebysql.openMini(\'' + url + '\')">'+value+'</a>';
			}else if(href_type=='4'){
				...
				rst = '<a class="LinkWord" href="javascript:queryTablebysql.downloadAttach(\'' + url + '\')">'+value+'</a>';
	...
	queryTablebysql.downloadAttach=function(url){
	...
	queryTablebysql.openWin=function(url){
		window.open(url);
	}
	queryTablebysql.jumpPage=function(url){
	...
	this.loadHead=function(){
		...
		if(this.headData != null){
			columnInfos = this.loadHeadData(this.headData);
		...
		$.ajax({
	        ...
	        success: function(data) {
	        	columnInfos = this.loadHeadData(data);
	        	this.columnInfos = columnInfos;
	        },
	        beforeSend: function() {
	...
	this.getSelectInput=function(){
	 ...
	 this.loadHeadData = function(data){
		...
		//ajaxqueryData 在ajaxRequest中使用
		this.ajaxRequestData = function(responseObj){
			var _this = this;
			var val = JSON.stringify(responseObj);
			_this.closeProgress();
			//异常处理
