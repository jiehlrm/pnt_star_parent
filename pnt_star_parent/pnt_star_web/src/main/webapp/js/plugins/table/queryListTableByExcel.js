...
queryListTableByExcel = function(){
	var temp = this;
	...
	this.writeTable = function() {
		//如果表头没有初始化则先进行初始化
		if(this.serviceName!=null&&this.serviceName!=undefined&&(this.tableHead==null||this.tableHead==undefined||tableHead=="")){
...
//			return false;
	...
	this.renderUrl=function(tempBean,colIdx,cellValue) {
		...
		if(info.name){
			...
			if(href_type=='1'){
				rst = '<a class="LinkWord" href="javascript:queryListTableByExcel.openWin(\'' + url + '\')">'+value+'</a>';
			}else if(href_type=='2'){
				rst = '<a class="LinkWord" href="javascript:queryListTableByExcel.jumpPage(\'' + url + '\')">'+value+'</a>';
			}else if(href_type=='3'){
				rst = '<a class="LinkWord" href="javascript:queryListTableByExcel.openMini(\'' + url + '\')">'+value+'</a>';
			}else if(href_type=='4'){
	...
	queryListTableByExcel.openWin=function(url){
		window.open(url);
	}
	queryListTableByExcel.jumpPage=function(url){
	...
	this.loadHead=function(){
		...
		var param = {"SERVICE_NAME":this.serviceName,
		...
		$.ajax({
	        ...
	        data: param,
	        dataType: "json",
	        url : nowurl+'ControlServlet.do?serviceName=commonServlet&methodName=getExcelHtmlHead',
	        success: function(data) {
	        	if(data["success"]){
	        		//表头信息
	    			 htmlHead = data.data.htmlHead;
	    			 gridcolumn=data.data.gridcolumn;
	    			this.tablehead=htmlHead;
	    			//表头的行数
	    			headRow=data.data.headRow;
	    			for(var i=0;i < gridcolumn.length; i++){
	    				var obj = gridcolumn[i];
	    				var info = new Object;
	    				if(obj.width==null||obj.width==0||obj.width=="0"){
	    					info["width"]=0;
	    				}else{
		...
		var table_head_tr=""; 
		 ...
		 if(this.selectMode!=null){
			 ...
			 if(this.selectMode=="single"){
				 table_head_tr+='<th width="45" rowspan="'+headRow+'"><p><span>选择</span></p></th>';
				}else{
					 table_head_tr+='<th width="45" rowspan="'+headRow+'"><p><span class="checkbox-check"><input type="checkbox" id="checkAll" /></span></p></th>';
				}
			 //替换掉服务器返回的第一行tr数据开始，把这个选择框加上<tr>
			 return table_colGroup_str+"<tbody>"+table_head_tr+ htmlHead.substring(4)+"</tbody>";
//			 return "<tbody>"+table_head_tr+ htmlHead.substring(4)+"</tbody>";
		 }else{
			this.columnNum=columnInfos.length;
		 }
		return table_colGroup_str+"<tbody>"+table_head_tr+"</tbody>";
//		 return "<tbody>"+table_head_tr+"</tbody>";
	}
