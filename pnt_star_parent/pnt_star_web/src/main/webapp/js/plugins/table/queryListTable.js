...
queryListTable = function(){
	...
	this.ajaxRequest = function(ops) {
		this.openProgress();
		//针对佣金报账做做处理
		if(ops.data.service_name != null && (ops.data.service_name == 'MSS_QUERY_MANU_WRITEOFFITEMS_1' 
			|| ops.data.service_name == 'MSS_QUERY_MANU_WRITEOFFITEMS_2')){
			reportFlag = true;
		}else{
			reportFlag = false;
		}
...
//				var doubleRowStyle = i % 2 == 0 ? '' : 'class="tableWRow"';
	...
	this.addRowClickEvent = function() {
		...
		if(this.rowClickFun) {
			...
			rows.click(function() {
				...
				selRowData['TABLE_OBJ'] = _this.table;
				selRowData['SELECT_MODE'] = _this.selectMode;
	 ...
	 * 添加行双击事件
	 */
	this.addRowDoubleClickEvent = function() {
		var _this = this;
		if(this.rowDoubleClickFun) {
			var thisObj = this;
			var rows = this.table.find('tr[rowEvent]');
			rows.css({cursor:"hand"});
			rows.dblclick(function() {
				...
				selRowData['TABLE_OBJ'] = _this.table;
				selRowData['SELECT_MODE'] = _this.selectMode;
				thisObj.rowDoubleClickFun(selRowData);
			});
		}
	}
...
//			return false;
	...
	this.init = function(tData) {
		...
		//初始化行双击事件（可选）
		this.rowDoubleClickFun = tData['rowDoubleClickFun'];
		//数据加载完成后执行的自定义方法（可选）
	...
	this.processRowFunDefault=function(i,tempBean){
		var rowhtml="";
		if(this.selectMode!=null || this.selectMode!= "null"){
		...
		for ( var i = 0; i <this.columnInfos.length; i++){
			...
			if(info["renderer"]=="onActionRenderer"){
				...
				rowhtml+=cellHtml;
			}else if(info["colType"]==7||info["colType"]=="7"){
					rowhtml+='<input type="text" style="text-align:center;" class="gy-input" value="'+cellValue+'">';
			}else if(info["type"]=="comboboxcolumn"){
				var dataArray = info["editor"].dataArray;
				var type = info["editor"].type;
				
				rowhtml+='<select style="width:107px;" class="gy-select table_select_q" value="'+cellValue+'></select>';
				for(var j=0;j<dataArray.length;j++){
					rowhtml+= '<option value="'+dataArray[j].id+'">'+dataArray[j].text+'</option>'
				}
				
				$(".table_select_q").each(function(){
					$(this).val(cellValue);
				});
			}else{
				//鼠标悬浮显示
				...
				if(reportFlag && i == 1){
					var OTHERSYSTEMMAINID = (tempBean.OTHERSYSTEMMAINID == null || 
							tempBean.OTHERSYSTEMMAINID == '')?"''":"'"+tempBean.OTHERSYSTEMMAINID+"'";
					var COMMISSITION_TYPE = (tempBean.COMMISSITION_TYPE == null || 
							tempBean.COMMISSITION_TYPE == '')?"''":"'"+tempBean.COMMISSITION_TYPE+"'";
					var STATUS = (tempBean.STATUS == null || 
							tempBean.STATUS == '')?"''":"'"+tempBean.STATUS+"'";
					
					var BUSI_TYPE = (tempBean.BUSITYPE == null || 
							tempBean.BUSITYPE == '')?"''":"'"+tempBean.BUSITYPE+"'";
					
					var BATCH_NO = (tempBean.BATCH_NO == null || 
							tempBean.BATCH_NO == '')?"''":"'"+tempBean.BATCH_NO+"'";
					var LATN_ID = $('#latn_id').val();
					rowhtml+='<button style="color:blue;" onclick="jumpDocumentDetails('+OTHERSYSTEMMAINID
						+','+COMMISSITION_TYPE+','+STATUS+','+BUSI_TYPE+','+BATCH_NO+','+LATN_ID+')">'+cellValue+'</button>';
				} else if(info['is_href']=='Y'){//判断是否为超链接
				...
				//悬浮显示
				if(!reportFlag || i != 0){
					rowhtml+='<div class="hide-text"><i class="jt-up"></i>'+cellValue+'</div>'+'</div>'
				}
//				rowhtml+='<div class="hide-text"><i class="jt-up"></i>'+cellValue+'</div>'+'</div>';
			}
	...
	this.renderUrl=function(tempBean,colIdx,cellValue) {
		...
		if(info.name){
			...
			if(href_type=='1'){
				rst = '<a class="LinkWord" href="javascript:queryListTable.openWin(\'' + url + '\')">'+value+'</a>';
			}else if(href_type=='2'){
				rst = '<a class="LinkWord" href="javascript:queryListTable.jumpPage(\'' + url + '\')">'+value+'</a>';
			}else if(href_type=='3'){
				rst = '<a class="LinkWord" href="javascript:queryListTable.openMini(\'' + url + '\')">'+value+'</a>';
			}else if(href_type=='4'){
				rst = '<a class="LinkWord" href="' + url + value+'" target="_blank" style="padding-left:20px;padding-right:50px;">'+value+'</a>';
			}else if(href_type=='5'){//增加下载超链接
				rst = '<a class="LinkWord" href="javascript:queryListTable.downloadAttach(\'' + url + '\')">'+value+'</a>';
			}else if(href_type=='6'){//弹框,openLayerwin在具体页面js中添加，方便控制窗体大小
				rst = '<a class="LinkWord" href="javascript:openLayerwin(\'' + url + '\')">'+value+'</a>';
			}else{
				rst = '<a class="LinkWord" href="javascript:queryListTable.openWin(\'' + url + '\')">'+value+'</a>';
			}
		}
	...
	queryListTable.downloadAttach=function(url){
		var a = document.createElement("a");  
	    a.setAttribute("href", encodeURI(encodeURI(url)));
	...
	queryListTable.openWin=function(url){
		window.open(url);
	}
	queryListTable.jumpPage=function(url){
	...
	this.loadHead=function(){
		...
		var param = {"service_name":"HEAD_QUERY_FIELDS",
		...
		$.ajax({
	        ...
	        data: param,
	        dataType: "json",
	        url : nowurl+'ControlServlet.do?serviceName=commonServlet&methodName=getTableHead',
	        success: function(data) {
	        	if(data.length > 0){
	    			...
	    			for(var i=0;i < data.length; i++){
	    				var obj = data[i];
	    				var info = new Object;
	    				if(obj.COL_LEN==null||obj.COL_LEN==0||obj.COL_LEN=="0"){
	    					info["width"]=0;
	    				}else{
	    				...
	    				if(obj.COL_TYPE == '4'){
	        				...
	        				if(obj.COM_TYPE=='1'){
	        					...
	        					var array = getData(obj.COMBOX_SERVICE_NAME,false,obj.COMBOX_BUSI_TYPE,null);
		...
		for(var i=0; i<$trs.length; i++){
			...
			if(selIdx!=null&&selIdx!=""){
				...
				if(this.selectMode=="multi"){
					...
					if($curTr.children(":first").find("p span").hasClass("radio-checked")){
						$curTr.find("input").each(function(){ //便利行内的input select的值
				        	selectedArray.push($(this).val());
				        });
						$curTr.find("select").each(function(){ //便利行内的input select的值
				        	selectedArray.push($(this).val());
				        });
						return selectedArray;
					}
