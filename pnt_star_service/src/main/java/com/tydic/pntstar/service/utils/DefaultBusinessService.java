...
//    			totalPoint=totalPoint*holidayDiscount;
	...
	public void minusPoint(Map<String,Object> param) {
		...
		dao.insert("InsertPointExchRecord", param);//新增一条积分兑换记录
		...
		for (Map map:goods) {
            ...
            map.put("extSysId", param.get("extSerialId"));
            map.put("extSerialId", param.get("extSerialId"));
            map.put("createStaff", param.get("createStaff"));
            dao.insert("InsertPointExchRecDetail",map);//新增一条积分兑换明细
		}
	}
	...
	public void exchUpdate(Map<String,Object> param,Map<String,Object> map, String exchType) {
		dao.update("UpdateExchPointAcctBalance",map);//更新积分账本表余额，可用余额的扣减
		dao.update("UpdateExchPointAcct",map);//更新积分账户表中的余额
		if("minus".equals(exchType)) {	
			...
			dao.insert("InsertExchPointPayout", map);//新增一条积分支出信息
		}
	}
