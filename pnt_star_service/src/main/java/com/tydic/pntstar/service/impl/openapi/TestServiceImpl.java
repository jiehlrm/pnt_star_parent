...
public class TestServiceImpl implements TestService {
	...
	public String testsql(String JsonStr) {
		...
		try {
            ...
            String sqlName=param.get("sqlName").toString();
            String flag=param.get("flag").toString();
            if("A".equals(flag)){
            	dao.insert(sqlName, param);
            }else if("Q".equals(flag)){
            	List<Map<String, Object>> list=dao.query(sqlName,param);            	
            }
            logger.info("hahhah");
			return Tools.buildResponseData(Tools.SUCCESS,"sql查询成功",null);
		}catch(Exception e) {
