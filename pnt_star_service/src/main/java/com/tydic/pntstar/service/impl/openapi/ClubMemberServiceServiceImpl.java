...
public class ClubMemberServiceServiceImpl implements ClubMemberServiceService{
	...
	public String addClubMemberService(String json) throws Exception {
		...
		String memberServiceId = dao.getPK("CLUB_MEMBER_SERVICE");
		try {
			jsonObj.put("memberServiceId", memberServiceId);//获取俱乐部会员服务表主键ID
			//新增俱乐部会员权益
			dao.insert("InsertClubMemberService", jsonObj);
			//---------------------会员服务目录位置mbrServiceCatalogLoc--具体业务待确认
			
			if(!Tools.isNull(jsonObj.get("catalogItemId"))) {
				String catalogLocId=dao.getPK("MBR_SERVICE_CATALOG_LOC");
				jsonObj.put("catalogLocId", catalogLocId);
				dao.insert("InsertMbrServiceCatalogLoc",jsonObj);
	        	
	        }
			//---------------------俱乐部服务关系clubServiceRel--具体业务待确认
//			jsonObj.put("clubServiceRelId", dao.getPK("CLUB_SERVICE_REL"));
//			dao.insert("InsertClubServiceRel", jsonObj);
			
			if(!Tools.isNull(jsonObj.get("relType"))) {
	    		String clubServiceRelId=dao.getPK("CLUB_SERVICE_REL");
	    		jsonObj.put("clubServiceRelId", clubServiceRelId);
	    		dao.insert("InsertClubServiceRel",jsonObj);
	        }
			
		} catch (Exception e) {
			return Tools.buildResponseData("1", "新增俱乐部会员权益失败", memberServiceId); 
		}
		return Tools.buildResponseData("0", "新增俱乐部会员权益成功", memberServiceId); 
	}
