...
//            param.put("busCode","积分兑换");
			//2：参数校验  必填校验,对兑换资源的校验
            // 参数校验开始  
      	    if(Tools.isNull(param.get("custId"))) {
              	throw new Exception("客户标识不能为空.");
            }
            ...
            param1.put("custId",param.get("custId").toString());
    		List<Map<String,Object>> accList=dao.query("QueryPointAcctListByCustId", param1);
